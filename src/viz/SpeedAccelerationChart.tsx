...
const SpeedAccelerationChartContent: React.FC<
  SpeedAccelerationChartContentProps
> = ({ data }) => {
  ...
  const filteredData = useMemo(
    () =>
      data
        .filter((v) => 2 <= v.speed)
        .map(({ speed, meanAcceleration, stdDevAcceleration }) => ({
          speed,
          meanAcceleration,
          confidenceAcceleration: [
            meanAcceleration - zScore * stdDevAcceleration,
            meanAcceleration + zScore * stdDevAcceleration,
          ],
        })),
    [data],
  ...
  return (
    <Box w="100%">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <ComposedChart width={500} height={300} data={filteredData}>
