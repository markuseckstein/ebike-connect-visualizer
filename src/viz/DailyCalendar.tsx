...
const colorForDistance = (distance: number) => {
  const colorsAndThresholds: [number, string][] = [
    [0, "green"],
    [10_000, "teal"],
    [25_000, "blue"],
    [50_000, "purple"],
...
const makeCalendar = (
  entries: [string, TargetStatistics][],
  year: number,
): ([string, TargetStatistics | null] | undefined)[][] => {
  const filteredEntries = Object.fromEntries(
    entries.filter(([date]) => extractYear(date) === year),
  );
  const weekSize = 7;
...
const renderButton = (
  value: [string, TargetStatistics | null] | undefined,
  now: Date,
) => {
  ...
  const isMissingData = MISSING_DATA.some(
    ({ range: { start, end } }) =>
      start.getTime() <= dateObj.getTime() && dateObj.getTime() < end.getTime(),
  );
  return (
    <Tooltip
      label={
        <Box textAlign="center">
          ...
          {!isMissingData ? (
            <>
              {`${stats?.count ?? 0} trip${stats?.count === 1 ? "" : "s"}`}
              <br />
              {`${((stats?.distance ?? 0) / 1000).toFixed(1)} km`}
            </>
          ) : (
            "Missing data"
          )}
    ...
    >
      <Button
        ...
        colorScheme={
          !isMissingData
            ? stats !== null
              ? colorForDistance(stats.distance)
              : "gray"
            : "red"
        }
        variant={isNow || isMissingData ? "outline" : undefined}
      />
    </Tooltip>
  );
};
...
const DailyCalendarContent: React.FC<DailyCalendarContentProps> = ({
  data,
}) => {
  const now = new Date();
  const entries = Object.entries(data).sort(([a], [b]) => (a < b ? -1 : 1)) as [
    string,
    TargetStatistics,
  ][];
  ...
  const [canDecrement, canIncrement] = [
    ...
    <VStack spacing={3} w="100%">
      ...
      <Box w="100%" overflow={{ base: "auto", "2xl": "visible" }}>
        <VStack
          spacing={1}
          maxW="100%"
          minWidth={{ base: "min-content", "2xl": "0%" }}
        >
          {calendar.map((rows, i) => (
...
export const DailyCalendar = () => {
  ...
  return <DailyCalendarContent data={data} />;
};
