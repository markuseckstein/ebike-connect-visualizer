...
const SpeedGearDistributionContent: React.FC<
  SpeedGearDistributionContentProps
> = ({ data: { gears, speedGearDistribution } }) => {
  ...
  const seriesData = speedGearDistribution.map(({ speed, values }) => ({
    ...Object.fromEntries(values.map((value, i) => [i, value * 100])),
    speed,
  }));
        ...
        >
          ...
          {gears.map((_, i) => (
            <Area
              ...
              stackId="1"
              stroke={strokeColor}
              fill={theme.colors.red[(10 - i - 1) * 100]}
            />
          ))}
        </AreaChart>
      </ResponsiveContainer>
