...
const DistributionContent: React.FC<DistributionContentProps> = ({
  data,
  heading,
  unit,
  strokeColor,
  ...
  const total =
    filtered.length > 0
      ? filtered.map(([_, value]) => value).reduce((a, b) => a + b)
      : 0;
  const seriesData = filtered.map(([name, value]) => ({
    name,
    value: (100 * value) / total,
  }));
  const xTicks = seriesData
    .map(({ name }) => name)
    .filter((bucket) => bucket % 10 === 0);
...
interface DistributionProps {
  dataKey: {
    [K in keyof Data]: Data[K] extends TargetBuckets ? K : never;
  }[keyof Data];
  heading: string;
  unit: string;
  strokeColor: string;
