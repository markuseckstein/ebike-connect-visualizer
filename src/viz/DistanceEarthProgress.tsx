...
const DistanceEarthProgressContent: React.FC<
  DistanceEarthProgressContentProps
> = ({ data }) => {
  ...
  return (
    <VStack w="100%">
      ...
      <Progress
        colorScheme="green"
        size="lg"
        value={nextRevolutionPercentage}
        w="100%"
      />
      <Box>
        {nextRevolutionPercentage.toFixed(1)}% to complete the distance around
        earth for the{" "}
        {nextRevolutionCount === 1
          ? "first"
          : `${nextRevolutionCount}${nextRevolutionCount === 2 ? "nd" : nextRevolutionCount === 3 ? "rd" : "th"}`}{" "}
        time
      </Box>
    </VStack>
  );
