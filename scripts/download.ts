...
type CommonActivityRide = Omit<
  ResponseActivityTripHeaders[0]["ride_headers"][0],
  "header_rides_ids"
>;
...
const timestampToISO = (timestamp: string) => {
  ...
  return (
    date
      ...
      .reverse()
      .join("") +
    date.toLocaleTimeString(LOCALE, { timeZone: TIMEZONE }).split(":").join("")
  );
};

const getFilenameForRide = (ride: CommonActivityRide) =>
  `${timestampToISO(ride.start_time)}-${ride.id}.json`;
...
const saveRide = (ride: ResponseActivityRide): boolean => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (contentToSave === existingContent) {
      console.log(
        `File ${filename} already exists and has the same content, ignoring`,
      );
      return false;
    } else {
      console.log(
        `!!! File ${filename} exists but content differs, updating (${existingContent.length} -> ${contentToSave.length})`,
      );
    }
  } else {
      ...
      for (const rideHeader of tripHeader.ride_headers) {
        ...
        if (!hasWritten) {
          ...
          if (totalNoop >= EXISTING_FILES_THRESHOLD) {
            console.log(
              `Reached a threshold of ${EXISTING_FILES_THRESHOLD} duplicate files, exiting`,
            );
            break loop;
          }
        }
