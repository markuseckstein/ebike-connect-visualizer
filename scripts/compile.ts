...
const loadInputs = () =>
  _.sort(
    fs
      ...
      .sort()
      .map((filename) =>
        fs.readFileSync(`${INPUT_DIRECTORY}/${filename}`, {
          encoding: "utf-8",
        }),
      )
...
const getMissingDataDays = (
  data: MissingDataRange[],
): (Omit<MissingDataRange, "range"> & { date: Date })[] =>
  data.flatMap(
    ({ range: { start, end }, distance, trips, altitude, calories, time }) => {
      const dates: Date[] = [];
...
const reduceInputStatistics = (
  data: ResponseActivityRide[],
  missingData: ReturnType<typeof getMissingDataDays>,
) => ({
  count: data.length,
  distance:
    _.sum(data, (d) => d.total_distance) +
    _.sum(missingData, (d) => d.distance),
});

const groupStatistics = (
  inputs: ResponseActivityRide[],
  groupBy: (date: string) => string,
) => {
  const grouped = _.group(inputs, (ride) => groupBy(ride.start_time));
  const missing = _.group(getMissingDataDays(MISSING_DATA), (d) =>
    groupBy(String(d.date.getTime())),
  );
  const sortedKeys = Array.from(
    new Set([...Object.keys(grouped), ...Object.keys(missing)]),
  ).sort();
  return Object.fromEntries(
    sortedKeys.map((key) => [
      key,
      reduceInputStatistics(grouped[key] ?? [], missing[key] ?? []),
    ]),
  );
};
...
const groupDistribution = (
  inputs: ResponseActivityRide[],
  groupBy: (ride: ResponseActivityRide) => number[],
  step: number,
) => {
  ...
  return _.list(_.min(buckets) ?? 0, _.max(buckets) ?? 0, (i) => [
    i * step,
    (record[i] ?? 0) / values.length,
  ]);
};
...
const targetStatisticsMonthly = (inputs: ResponseActivityRide[]) =>
  groupStatistics(inputs, (date) =>
    timestampToIso(date).split("-").slice(0, 2).join("-"),
  );
...
const targetStatisticsWeekly = (inputs: ResponseActivityRide[]) => {
  ...
  const dailyDistances: { count: number; distance: number }[][] = _.list(
    0,
    WEEKDAYS.length - 1,
    () => [],
  );
  Object.entries(daily).forEach(([isoDate, values]) =>
    dailyDistances[
      (WEEKDAYS.length + new Date(isoDate).getDay() - 1) % WEEKDAYS.length
    ].push(values),
  );
  ...
  return Object.fromEntries(
    WEEKDAYS.map((label, i) => {
      ...
      const countAndDistances = countAndDistancesBase.concat(
        _.list(0, totalWeeks - countAndDistancesBase.length - 1, () => ({
          count: 0,
          distance: 0,
        })),
      );
      const count = computeMeanStdDev(countAndDistances.map((v) => v.count)),
        distance = computeMeanStdDev(countAndDistances.map((v) => v.distance));
      return [
        label,
        {
...
const targetCumulativeDistance = (inputs: ResponseActivityRide[]) => {
  ...
  daily.forEach(([key, { distance }]) => {
    if (array.length > 0 && array[array.length - 1][0] === key) {
      ...
      array.push([
        key,
        (array.length > 0 ? array[array.length - 1][1] : 0) + distance,
      ]);
    }
  });
...
const targetRecords = (inputs: ResponseActivityRide[]) => ({
  totalDistance:
    _.sum(inputs, (input) => input.total_distance) +
    _.sum(MISSING_DATA, (d) => d.distance),
  maxSpeed: _.max(inputs.map((input) => input.max_speed)) ?? 0,
  totalAltitudeGain:
    _.sum(inputs, (input) => input.elevation_gain) +
    _.sum(MISSING_DATA, (d) => d.altitude),
  tripsCount: inputs.length + _.sum(MISSING_DATA, (d) => d.trips),
  totalCalories:
    _.sum(inputs, (input) => input.calories) +
    _.sum(MISSING_DATA, (d) => d.calories),
  totalOperationTime:
    _.sum(inputs, (input) => parseInt(input.operation_time)) +
    _.sum(MISSING_DATA, (d) => d.time * 60 * 60 * 1000),
});
...
const targetCadence = (inputs: ResponseActivityRide[]) =>
  groupDistribution(
    inputs,
    ({ cadence }) =>
      cadence.flatMap((array) => array.map((v) => (v !== null ? v : -1))),
    1,
  );
...
const targetSpeed = (inputs: ResponseActivityRide[]) =>
  groupDistribution(
    inputs,
    ({ speed }) =>
      speed.flatMap((array) => array.map((v) => (v !== null ? v : -1))),
    1,
  );
...
const targetPower = (inputs: ResponseActivityRide[]) =>
  groupDistribution(
    inputs,
    ({ power_output }) =>
      power_output.flatMap((array) => array.map((v) => (v !== null ? v : -1))),
    5,
  );
...
const targetGears = (inputs: ResponseActivityRide[]) => {
      ...
      _.zip(a.cadence, a.speed)
        ...
        .flatMap(([cadences, speeds]) =>
          _.zip(cadences, speeds)
            .filter(
              ([cadence, speed]) =>
                cadence !== null && speed !== null && cadence > 0 && speed > 0,
            )
            .map(([cadence, speed]) => {
  ...
  const step = 0.05;
  const ratiosRecord = _.counting(metersPerRotations, (value) =>
    Math.round(value / step),
  );
  const minimum = Math.round((_.min(metersPerRotations) ?? 0) / step),
    maximum = Math.round((_.max(metersPerRotations) ?? 0) / step);
  const result = _.list(minimum, maximum, (i) => {
    const bucket = i * step;
    const value = ratiosRecord[i] ?? 0;
    return [bucket, value];
  });
  const slopes = _.zip(result.slice(0, result.length - 1), result.slice(1)).map(
    ([[, v0], [, v1]]) => (v1 - v0) / step,
  );
  const radius = 5,
    decrease = 0.75,
  ...
  for (let i = 0; i < slopes.length - 1; i++) {
    if (
      slopes[i] > 0 &&
      slopes[i + 1] < 0 &&
      (result[i + 1 - radius] === undefined ||
        result[i + 1 - radius][1] < decrease * result[i + 1][1]) &&
      (result[i + 1 + radius] === undefined ||
        result[i + 1 + radius][1] < decrease * result[i + 1][1]) &&
      minRatio < result[i + 1][0] &&
  ...
  const closestTo = (i: number): [number, number][] =>
    means
      .map<[number, number]>((mean, id) => [id, Math.abs(i - mean)])
      .sort(([, a], [, b]) => a - b);
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (
      closest[0][0] === classifiedGears[i] &&
      i <= means[classifiedGears[i]] === i <= means[closest[1][0]] &&
      closest[1][1] < closest[0][1] * 2 * maxRadiusSymmetry
    ) {
  ...
  const gearDistribution = means.map(
    (key) => [key, result[key][0], (gearsCounts[key] ?? 0) / total] as const,
  );
  ...
  const speedGearDistribution = _.list(
    _.min(speedKeys) ?? 0,
    _.max(speedKeys) ?? 0,
    (speedKey) =>
      [speedKey * speedStep, speedGroups[speedKey] ?? []] satisfies [
        number,
        typeof values,
      ],
  ).map(([speed, items]) => {
    const gearsGroups = _.counting(
      items,
      (o) =>
        classifiedGearsClean[
          Math.round(o.metersPerRotation / step) - minimum
        ] ?? -1,
    );
    const gearsVector = _.list(gears.length - 1).map(
      (i) => gearsGroups[i] ?? 0,
    );
    const totalInGroup = _.sum(gearsVector);
    const normalizedVector = gearsVector.map((v, i) =>
      totalInGroup !== 0 ? v / totalInGroup : i === gears.length - 1 ? 1 : 0,
    );
...
const targetAcceleration = (inputs: ResponseActivityRide[]) => {
  ...
  inputs.forEach((input) => {
    ...
    if (Math.abs(dtSingleSecond - 1) > 0.05) {
      // Data points occur every second, we tolerate a 5% error
      return;
    }
    const dt = 2 * stepRadius * dtSingleSecond;
    for (let i = stepRadius; i < speeds.length - stepRadius; i++) {
      const speedBefore = speeds[i - stepRadius],
        speed = speeds[i],
        speedAfter = speeds[i + stepRadius];
      const acceleration = (speedAfter - speedBefore) / dt;
      speedsAndAccelerations.push({
        speed,
  ...
  speedsAndAccelerations.forEach(({ speed, acceleration }) => {
    const bucket = Math.min(
      Math.floor((totalPoints * speed) / maxSpeed),
      totalPoints - 1,
    );
